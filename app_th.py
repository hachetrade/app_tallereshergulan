...
def resource_path(relative_path):
    ...
    try:
        # If we are running in a bundle created by PyInstaller
        base_path = sys._MEIPASS
    except AttributeError:
        # If we are running in development (not bundled by PyInstaller)
...
def populate_articulos_odoo(input_file, label_widget):
    headers = ['id', 'name', 'product_tag_ids', 'standard_price', 'type', 'sale_ok', 'is_storable', 'seller_ids', 'seller_ids/price',
    ...
    for index, row  in df.iterrows():
        ...
        pop_line['is_storable']= 'TRUE' if getattr(label_widget, 'tipo articulo', None) == 'Bienes' else 'FALSE'
        ...
        for page in pdf.pages:
            ...
            for line in lines:
                ...
                if matches:
                    ...
                    if match_articulo and match_precio:
                        articulo = match_articulo.group(1)
                        raw_precio = match_precio.group(1).replace('.', '').replace(',', '.').replace(' €', '')
...
def convert_ebakilan_to_excel(pdf_path, output_path, uds_albaran):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            lines = text.split('\n')
            for line in lines:
                albaran_match = re.match(pattern_alb_num, line)
                if albaran_match:
                    albaran = albaran_match.group(1).strip("")
                    pattern = fr'{albaran} - (\d+) (\S+) (\d+)'
                # else:
                    # albaran = input(f'No puedo leer el Nª de albarán, por favor introduzca el Nº de Albarán: ')

                matches = re.match(pattern_10, line)

                if matches and albaran:
...
def ask_for_units(label_widget):
    units = simpledialog.askinteger("Input", "La oferta/presupuesto es para cuantas unidades?:", minvalue=1, parent=root)
...
def upload_and_process_bom():
    ...
    if pdf_path:
        ...
        if output_path:
            ...
            if success:
                messagebox.showinfo("Success", f"LDM convertida y guardada en {output_path}")
                if errors:
...
def upload_and_process_cmo_pres(label_widget):
    print(label_widget.file_path, label_widget.units)
    pdf_path = label_widget.file_path
    units = getattr(cmo_pres_label, 'units', None)
    print(pdf_path, units)
    if pdf_path and units:
        output_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")])
        if output_path:
...
def upload_and_process_ebakilan(label_widget):
    ...
    if pdf_path and units:
        ...
        if output_path:
            ...
            messagebox.showinfo("Success", f"EBAKILAN lista de componentes procesado y guardado en {output_path}")
...
def create_app():
    ...
    icon_path =  resource_path('logos/BonT.ico')
